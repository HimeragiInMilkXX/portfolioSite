...
const HighlightsBlock: FunctionComponent<{ classes?: string, setShowInstruction: ( value: React.SetStateAction<boolean> ) => void, name: string }> = ( { classes, setShowInstruction, name } ) => {
    ...
    return (
        ...
        <div onMouseEnter={ () => setShowInstruction(false) } onMouseLeave={ () => setShowInstruction(true) } className={`${classes} hover:[&>section]:opacity-100! highlightsBlock w-1/3 transition-all hover:[&>img]:h-105 hover:w-5/6 hover:[&>img]:shadow-2xl hover:[&>img]:blur-xs hover:[&>img]:opacity-30 relative shadow-2xl`}>

            <HoverTextImage url="/home/highlights" project={name} imgClasses="min-h-70 h-full" sectionClasses="left-1/6! w-3/5!"/>
